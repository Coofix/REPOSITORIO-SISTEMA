<?php
    ...
    if(isset($_POST['incluir_paciente'])){
        ...
        if(count($erro) == 0){
            ...
            //$db = new PDO($host, $usuario, $senha, $bd);
            //$paciente_id = $bd->lastInsertId();
   ...
   // if(isset($_POST['incluir_responsavel'])){
        
     //   $paciente_id = intval($_POST['id_paciente']);

    //    $sql_code = "SELECT nome_cliente FROM CLIENTE WHERE id_cliente = '$_SESSION[id_cliente]'";
    //    $nome_cliente = $sql_code;
        //$nome_cliente = $mysqli->query($sql_code) or die($mysqli->error);
